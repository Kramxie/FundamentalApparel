...
if (!orderId) {
  console.error('Usage: node list_inventory_txs.js --orderId <orderId>'); process.exit(2);
}
...
async function main() {
  ...
  let query = { orderId: null };
  try { query = { orderId: ObjectId(orderId) }; } catch (e) { query = { orderId: orderId }; }
  ...
  txs.forEach(t => console.log(JSON.stringify({ _id: t._id, type: t.type, qty: t.qty, sizesMap: t.sizesMap, inventory: t.inventory, inventoryName: t.inventoryName, createdAt: t.createdAt }, null, 2)));
